#!/usr/bin/env bash
# ==============================================================================
# FEDORA COSMIC DESKTOP MASTER SETUP SCRIPT (RUST-POWERED COSMIC DE)
# ==============================================================================
set -e
...
LOGFILE="/var/log/fedora_cosmic_setup.log"
...
echo "[$(date -Iseconds)] 🚀 Starting Fedora COSMIC Desktop Setup for $REAL_USER ($USER_HOME)..."

# ------------------------------------------------------------------------------
# 1. ENABLE COSMIC REPOSITORIES & RPM FUSION
# ------------------------------------------------------------------------------
echo "📦 Enabling COPR repositories for COSMIC Desktop & RPM Fusion..."
...
# Enable COSMIC COPR repositories
sudo dnf copr enable -y @system76/cosmic 2>/dev/null || sudo dnf copr enable -y ryanabx/cosmic 2>/dev/null || true
sudo dnf copr enable -y imput/helium 2>/dev/null || true
sudo flatpak remote-add --if-not-exists flathub https://dl.flathub.org/repo/flathub.flatpakrepo || true

# ------------------------------------------------------------------------------
# 2. INSTALL COSMIC DESKTOP & ESSENTIAL SUITE
# ------------------------------------------------------------------------------
echo "🌌 Installing COSMIC Desktop Environment & Applications..."
sudo dnf install -y --allowerasing --skip-unavailable --nogpgcheck \
    cosmic-desktop cosmic-comp cosmic-panel cosmic-launcher cosmic-term cosmic-files cosmic-settings cosmic-applets cosmic-greeter cosmic-bg \
    helium-bin sddm sddm-kcm thunar thunar-archive-plugin grim slurp wl-clipboard cliphist \
...
# ------------------------------------------------------------------------------
# 3. SCAN LOCAL ARCHIVES & INSTALL DAVINCI RESOLVE + PROTONUP-QT
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# 4. CONFIGURE DESKTOP LAUNCHERS & WRAPPERS
# ------------------------------------------------------------------------------
...
# Enable COSMIC Display Manager / SDDM
sudo systemctl enable --now cosmic-greeter 2>/dev/null || sudo systemctl enable --now sddm 2>/dev/null || true
sudo systemctl set-default graphical.target

sudo chown -R "$REAL_USER:$REAL_USER" "$USER_HOME/.config" "$USER_HOME/.local" "$USER_HOME/.icons" "$MEMLOG" 2>/dev/null || true

echo "[$(date -Iseconds)] 🎉 Fedora COSMIC Desktop Setup completed successfully!" >> "$MEMLOG"
echo "========================================================================"
echo " 🎉 COSMIC Setup Completed! Logged to: $MEMLOG"
echo "========================================================================"
