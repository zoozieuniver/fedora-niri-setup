...
LOGFILE="/var/log/install_my_apps.log"
...
PROTONUP_APPIMAGE=$(find "$SCRIPT_DIR" "$USER_HOME/Downloads" "$USER_HOME/Завантаження" ./ /tmp /run/media -iname "ProtonUp-Qt-*.AppImage" 2>/dev/null | head -n 1 || true)
...
DAVINCI_ZIP=$(find "$SCRIPT_DIR" "$USER_HOME/Downloads" "$USER_HOME/Завантаження" ./ /tmp /run/media -iname "DaVinci_Resolve_*.zip" 2>/dev/null | head -n 1 || true)
...
# ------------------------------------------------------------------------------
# 4. CONFIGURE LAUNCHERS & WRAPPERS
# ------------------------------------------------------------------------------
...
Exec=/home/$REAL_USER/.local/bin/zed %F
Icon=zed
Terminal=false
...
Exec=/home/$REAL_USER/.local/bin/protonup-qt
Icon=protonup-qt
Terminal=false
